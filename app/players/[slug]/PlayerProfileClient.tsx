...
export default function PlayerProfileClient({ player }: { player: PlayerProfile }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      {/* Back Navigation */}
      <Link href="/players" className="inline-flex items-center gap-2 px-3.5 py-2 bg-white/[0.04] border border-white/[0.08] text-xs font-semibold text-[#858585] hover:text-white mb-8 transition-colors">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-[400px] flex-shrink-0">
          <div className={`bg-[#080808] border relative overflow-hidden ${playerBan.isBanned ? "border-red-900/60" : "border-white/[0.04]"}`}>

            <div className="p-8 relative z-10">
              {/* Avatar Container */}
              <div className="w-32 h-32 bg-[#0a0a0a] border border-white/10 overflow-hidden flex items-end justify-center mx-auto mb-6 shadow-2xl">
                {player.avatarUrl ? (
              ...
              <div className="text-center">
                ...
                <div className="flex flex-col gap-2 mb-8">
                  ...
                  {player.discordUrl && (
                    player.discordUrl.startsWith('http') ? (
                      <a href={player.discordUrl} target="_blank" rel="noreferrer" className="flex items-center justify-between px-4 py-2.5 bg-white/[0.02] hover:bg-white/[0.06] transition-colors border border-white/[0.04] text-[#888888] hover:text-[#5865F2] text-[11px] font-mono uppercase tracking-widest group">
                        <div className="flex items-center gap-3"><DiscordIcon className="w-4 h-4" /> Discord</div>
                        <ExternalLink className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" />
                      </a>
                    ) : (
                      <div className="flex items-center justify-between px-4 py-2.5 bg-white/[0.02] border border-white/[0.04] text-[#888888] text-[11px] font-mono uppercase tracking-widest">
                ...
                <div className="border-t border-white/[0.04] pt-6 flex flex-col gap-6 text-left">
                  {/* Current Team Status */}
                  <div>
                    ...
                    {player.currentTeam ? (
                      <Link
                        ...
                        className="flex items-center gap-4 group p-3 bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.05] transition-colors"
                      >
                      ...
                      <div className="flex items-center gap-3 px-4 py-3 bg-white/[0.02] border border-white/[0.04] text-white font-bold text-sm uppercase tracking-widest">
        ...
        <div className="flex-1 space-y-8">
          <div className="bg-[#080808] border border-white/[0.04] p-8 shadow-xl">
            ...
            {player.history.length > 0 ? (
              <div className="relative border-l-2 border-white/[0.05] ml-3 space-y-6">
                    ...
                    <div key={`${hist.teamId}-${hist.joinedAt}-${idx}`} className="relative pl-6">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        <div className="flex items-start gap-4">
                          <Link href={`/teams/${hist.teamSlug}`} className="w-10 h-10 bg-[#0a0a0a] border border-white/10 p-1 flex-shrink-0 hover:border-purple-500/50 transition-colors">
                        ...
                        <div className="text-[10px] font-mono font-bold text-[#666] uppercase bg-white/[0.02] border border-white/[0.05] px-2.5 py-1">
